...
> standard read is **one section doc + 01 Core Facts**, never the whole corpus.
...
2. **Read the matching section doc + `01 Core Facts`.** That pair is the standard read — the
   section doc holds the Q&A entries; Core Facts holds the canonical numbers they cite. Resolve any
...
- **One draft per thread.** If a draft already exists, don't create a second — list it as
  already-drafted.
- **Don't read the whole KB.** One section doc + `01 Core Facts` is the standard read. If you find
