...
**If the artifact lives at a GitHub URL** (other repo, gist, raw URL):
...
**If the artifact only lives locally** (just produced by `/skill-creator`), inline base64:
...
Exactly one of `skill_md_url` or `skill_md_b64` — the workflow rejects payloads with both or neither.
