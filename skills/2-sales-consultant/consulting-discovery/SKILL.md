...
```bash
npx skills add https://github.com/peter-tu-zynkr/zynkr-skill-builder --skill consulting-discovery
```
...
Launch the `pain-point-discovery` agent (`./agents/pain-point-discovery.md`) using the Agent tool.
...
- Adjust → reopen pain-point-discovery to refine
```
...
Launch the `company-vision-discovery` agent (`./agents/company-vision-discovery.md`) using the Agent tool, passing `STAGE1_SUMMARY` as context.
