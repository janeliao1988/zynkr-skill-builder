...
author: Peter Tu
description: >-
  Files loose recordings sitting in Peter's Google Drive recording inbox into
  ...
  it belongs in, shows Peter a filing plan to approve, then moves the whole
  group and logs the move to the `[@] Video Index` sheet. If nothing fits it
  proposes a new sub-folder. Trigger on /admin-video-document, '整理錄影檔', '歸檔錄影',
  '歸檔影片到資料夾', '整理錄影資料夾', 'file my recordings', 'sort the recording folder',
  'organise the recordings folder', or when Peter points at the recording inbox
  and wants the videos filed.
input: >-
...
output: >-
  ...
  + Chat + SRT together), one new row per unit in the `[@] Video Index` sheet,
  and a zh-TW run report.
ipoProvenance:
  input: frontmatter
  process: frontmatter
  output: frontmatter
