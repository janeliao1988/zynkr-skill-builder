...
description: >-
  ...
  dashboard", "draft the project dashboard", "寄週報給團隊", "/project-status-weekly",
...
input: >-
  A project-tracker Google Sheet (a 專案管理總表 task tab) you configure, plus a
  recipient list. Optionally an explicit 'as of' date.
process: >-
  Anchor on today → read the tracker tab → diff against last week's report →
  reason over status/dates to derive health, blockers, this-week actions, %
  complete, next milestone, decisions, timeline → render styled HTML → create a
  Gmail draft (never send).
output: >-
  A Gmail DRAFT (never auto-sent) containing the 專案狀態看板 weekly dashboard as a
  styled HTML email, addressed to a configurable recipient list.
ipoProvenance:
  input: frontmatter
  process: frontmatter
  output: frontmatter
...
synergy:
  - '3.10'
installCommand: curl -sL zynkr.ai/s/3.10.md -o ~/.claude/skills/project-status-weekly.md
updatedAt: '2026-06-02'
firstSeen: '2026-06-01'
sourceRepo: 'https://github.com/peter-tu-zynkr/zynkr-skill-builder'
sourceFile: 3-operations/project-status-weekly/SKILL.md
---
